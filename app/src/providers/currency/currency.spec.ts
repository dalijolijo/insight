...
describe('CurrencyProvider', () => {
  ...
  it('initialises', () => {
    ...
    expect(currency.defaultCurrency).toBe('BCH');
    expect(currency.currencySymbol).toBe('BCH');
  ...
  it('sets currency by updating the symbol and changing the multiplication factor', () => {
    ...
    currency.setCurrency('mBCH');
    expect(currency.currencySymbol).toBe('mBCH');
  ...
  it('gets proper conversion after changing currency', () => {
    ...
    expect(currency.getConversion(aFloat)).toBe('12345.09876543 BCH');

    currency.setCurrency('mBCH');
    expect(currency.getConversion(aFloat)).toBe('12345098.76543 mBCH');
